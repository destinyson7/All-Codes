...
void solve()
{
    ...
    for(ll i = 1; i <= k - 1; i++)
    {
        ...
        for(ll j = 1; j <= c; j++)
        {
            bool car_here = ((upsum[k][j] - upsum[k - i][j]) == 0);

            car_here |= ((upsum[min(r + 1, k + i)][j] - upsum[k][j]) < i);
    ...
    for(ll i = 1; i <= r - k; i++)
    {
        ...
        for(ll j = 1; j <= c; j++)
        {
            bool car_here = ((downsum[k][j] - downsum[k + i][j]) == 0);

            car_here |= ((downsum[max(0LL, k - i)][j] - downsum[k][j]) < i);
